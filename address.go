...
func (a *Address) parse() (err error) {
	...
	m = regexp.MustCompile(`(?si)^(.*?((республика(\s*,|\s+Марий\s*Эл|\s+Саха\s*\(Якутия\)|.+?Алания|\s+[а-я]+)|РСО-Алания|республики.*?,)|(область\s*-\s*Кузбасс*|области|область|бласть|обл\.|край|округ|ЕАО|ЯНАО)[\s,]))`).FindStringSubmatch(res)
	if len(m) > 0 {
